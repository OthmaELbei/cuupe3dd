...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/21 09:57:37 by oelbied           #+#    #+#             */
/*   Updated: 2025/08/21 09:58:44 by oelbied          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void makein_mine_maps(t_datagame *game)
{
    ...
    int menimaps = 150; // حجم المينيماب
    int minimapx = WIDIH - menimaps - 20; // نخليه بعيد شوي من الحافة
    int minimapy = 20;
    int player_x = minimapx + menimaps / 2;
    int player_y = minimapy + menimaps / 2;

    float scale = (float)menimaps / (TILE_SIZE * 12); // موازنة أفضل للرسم

    // خلفية المينيماب (شبه شفافة)
    draw_pixel(&game->img, minimapx - 2, minimapy - 2, menimaps + 4, menimaps + 4, 0x222222);

    // إطار المينيماب
    draw_line(&game->img, minimapx - 2, minimapy - 2, minimapx + menimaps + 2, minimapy - 2, 0xFFFFFF);
    draw_line(&game->img, minimapx - 2, minimapy + menimaps + 2, minimapx + menimaps + 2, minimapy + menimaps + 2, 0xFFFFFF);
    draw_line(&game->img, minimapx - 2, minimapy - 2, minimapx - 2, minimapy + menimaps + 2, 0xFFFFFF);
    draw_line(&game->img, minimapx + menimaps + 2, minimapy - 2, minimapx + menimaps + 2, minimapy + menimaps + 2, 0xFFFFFF);

    int view_range = 8; // عدد المربعات اللي تبان حول اللاعب
    int player_tile_x = (int)(game->player.x / TILE_SIZE);
    int player_tile_y = (int)(game->player.y / TILE_SIZE);

    for (int y = player_tile_y - view_range; y <= player_tile_y + view_range; y++)
    {
        for (int x = player_tile_x - view_range; x <= player_tile_x + view_range; x++)
        {
            if (y >= 0 && y < game->row && x >= 0 && x < game->colone)
            {
                int tile_x = minimapx + (x - player_tile_x) * TILE_SIZE * scale + menimaps / 2;
                int tile_y = minimapy + (y - player_tile_y) * TILE_SIZE * scale + menimaps / 2;
                int tile_size = TILE_SIZE * scale;

                if (game->map[y][x] == '1')
                    draw_pixel(&game->img, tile_x, tile_y, tile_size, tile_size, 0xAAAAAA); // جدران
                else
                    draw_pixel(&game->img, tile_x, tile_y, tile_size, tile_size, 0x333333); // أرضية
            }
        }
    }

    // اللاعب (أحمر واضح)
    draw_sercel_player(&game->img, player_x, player_y, PLAYER_SIZE * scale, 0xFF0000);

    // اتجاه اللاعب (أزرق سماوي)
    int line_end_x = player_x + cos(game->player.rotation_angle) * 30;
    int line_end_y = player_y + sin(game->player.rotation_angle) * 30;
    draw_line(&game->img, player_x, player_y, line_end_x, line_end_y, 0x00FFFF);

    // // رسم بعض الأشعة فقط باش يبان الاتجاه
    // int i = 0;
    // while (i < NUM_RAYS)
    // {
    ...
    //     if (ray->distance != INFINITY)
    //     {
    //         int ray_end_x = minimapx + (ray->wall_hit_x - game->player.x) * scale + menimaps / 2;
    //         int ray_end_y = minimapy + (ray->wall_hit_y - game->player.y) * scale + menimaps / 2;
...
//     int menimaps = 130;
//     int minimapx = WIDIH - menimaps * 3;
//     int minimapy = 80;
//     int player_x;
//     int player_y;

//     float scale = (float)menimaps / game->windo_with * 2;

//     // خلفية المينيماب
//     draw_pixel(&game->img, minimapx - 2, minimapy - 2, menimaps + 4, menimaps + 4, 0x1a1a1a);

//     // اللاعب في الوسط => نحدد المركز
//     player_x = minimapx + menimaps / 2;
//     player_y = minimapy + menimaps / 2;

//     // حجم المنطقة التي نعرضها (مثلا 10x10 تايلز حول اللاعب)
//     int view_range = 10;
...
//                 else
//                     draw_pixel(&game->img, tile_x, tile_y, tile_size, tile_size, 0x000000);
//             }
//         }
//     }

//     // اللاعب (دائرة صغيرة في الوسط)
//     draw_sercel_player(&game->img, player_x, player_y, PLAYER_SIZE * scale, 0xFF00FF);

//     // اتجاه اللاعب (خط قصير)
//     int line_end_x = player_x + cos(game->player.rotation_angle) * 40;
//     int line_end_y = player_y + sin(game->player.rotation_angle) * 40;
//     draw_line(&game->img, player_x, player_y, line_end_x, line_end_y, 0x0fafa1);

//     // رسم بعض الأشعة (اختياري)
//     int i = 0;
//     while (i < NUM_RAYS)
//     {
//         t_ray *ray = &game->rays[i];
